	...
	set segment_time=%2
		if NOT DEFINED !segment_time! set /P segmenttime="Enter segment_time: "
...
rem	IF NOT !ERRORLEVEL! EQU 02 goto:eof
		...
		for %%f in (%ree%\*split*!input!.mkv) do (
			...
			echo 	-i %%f -c copy -f segment -segment_time !segment_time! -reset_timestamps 1
			echo 		!newInput!.%%06d.mkv
echo	--------------------------------------------------------------------------------
			ffmpeg -v quiet -stats -i %%f -c copy -f segment -segment_time !segment_time! -reset_timestamps 1 !newInput!.%%06d.mkv
		)
		for %%f in (%ree%\*split*!input!_new.mkv) do (
			echo ffmpeg -v quiet -stats
			echo 	-i %%f -c copy -f segment -segment_time !segment_time! -reset_timestamps 1
...
echo	--------------------------------------------------------------------------------
			ffmpeg -v quiet -stats -i %%f -c copy -f segment -segment_time !segment_time! -reset_timestamps 1 !newInput!.%%06d_new.mkv
		)
		echo.
		:eof