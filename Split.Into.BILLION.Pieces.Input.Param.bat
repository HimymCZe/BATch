...
echo	md !output!\%%~nf
echo	----------------------------------------  -re deleted ----------------------------------------
	ffmpeg -v quiet -stats -i %%f -c copy %2 %3 %4 %5 %6 %7 %8 -f segment -segment_time 50 -reset_timestamps 1 !output!\%%~nf\Split.%%06d.mkv -y
...
rem	echo.
	...
	CHOICE /T 90 /C 01 /D 0 /M "Spustit Doubale.BATCH.eNVenc.encode? (1:YES)"
	IF !ERRORLEVEL! EQU 02 EncodeEntireFOLDER.DOUBLE.BATCH.inputfolder.quality.MORE.bat !output!\%%~nf
)	
rem	timeout /t 99
	exit
:eof