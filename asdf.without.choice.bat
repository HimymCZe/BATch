...
rem		CHOICE /N /C 12468A0 /M "Continue?: "
rem			IF !ERRORLEVEL! EQU 07 color 70 & goto :eof
rem			IF !ERRORLEVEL! EQU 06 color 70 & set HEVC.Splits=UnEncoded.AVC.Source
rem			IF !ERRORLEVEL! EQU 05 color 70 & set HEVC.Splits=HEVC.Splits.NvEnc.MiXeD.600
rem			IF !ERRORLEVEL! EQU 04 color 70 & set HEVC.Splits=HEVC.Splits.NvEnc.30.36.42.600
rem			IF !ERRORLEVEL! EQU 03 color 70 & set HEVC.Splits=HEVC.Splits.NvEnc.30.34.38.600
rem			IF !ERRORLEVEL! EQU 02 color 70 & set HEVC.Splits=HEVC.Splits.NvEnc.30.32.34.600
rem			IF !ERRORLEVEL! EQU 01 color 70 & set HEVC.Splits=HEVC.Splits.CPU.CRF26.600
		color 70 & set HEVC.Splits=HEVC.Splits.NvEnc.30.32.34.600
...
echo -------------------------------- DELETING TEMPs --------------------------------
	...
	for /R %%f in (*m4a *txt *avs *ffindex *srip *flac *txt *hevc *h265) do (
rem		echo deleting... "%%f"
		del "%%f"
	)
rem	timeout /T -1
...
rem	timeout /T -1
	"C:\Program Files\WinRAR\Rar.exe" m -m0 !output!\!inputfoldername!_!HEVC.Splits!.log.rar *log
...
rem			echo FINAL.MERGE !line!
				"c:\Users\asdf\OneDrive\Desktop\mkvtoolnix (old)\mkvmerge.exe" -o "!output!\!inputfoldername!_!HEVC.Splits!.mkv" !line:~2!
	)
	if !centcount! == 0 ren !output!\!inputfoldername!_Output.000000.mkv !inputfoldername!_!HEVC.Splits!.mkv

echo -------------------------------- FINAL CLEAN UP --------------------------------

for /L %%n in (0,1,!centcount!) do (
	echo deleting... "!output!\!inputfoldername!_Output.000%%n00.mkv"
	del "!output!\!inputfoldername!_Output.000%%n00.mkv"
)

echo --------------------------------------------------------------------------------
rem	msg %username% Encoding !input! DONE.
	CD /D c:\Users\asdf\Desktop\BBC\
:oef